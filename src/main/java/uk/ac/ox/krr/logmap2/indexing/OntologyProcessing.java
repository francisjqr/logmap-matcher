 ...
 * 
 *
 * @author Ernesto Jimenez-Ruiz Created: Sep 12, 2011
 *
 */
public class OntologyProcessing {
	...
	/* Exact match entries */
	// protected Map<String, Integer> extact_occurrence_entries = new
	...
	// protected Map<Set<String>, Set<Integer>> invertedFileWeakLabelsL2 = new
	// HashMap<Set<String>, Set<Integer>>();
	// protected Map<Set<String>, Set<Integer>> invertedFileWeakLabelsL3 = new
	// HashMap<Set<String>, Set<Integer>>();
	...
	// private HashMap<Integer, Set<Integer>> identifier2directkids= new
	// HashMap<Integer, Set<Integer>>();
	// private Map<Integer, Set<Integer>> identifier2directparents= new
	// HashMap<Integer, Set<Integer>>();

	/**
	 * This set will be used to propagate equivalences (entities store equivalents)
	 */
	// TODO this set will need to be enriched with anchors... or at leats considered
	// after indexing
	// In index
	...
	// private Map<Set<Integer>, Integer> generalHornAxioms= new
	// HashMap<Set<Integer>, Integer>();

	/** We use this structure to create weak labels */
	// private Map<Integer, List<String>> identifier2stemmedlabel = new
	...
	// Must be kept outside index to be removed after use and to attached to onto
	// (i.e ontoprocessing object)
	...
	// TranslatorManager.getCurrentTranslatorObject();
	private TranslatorManager translator = new TranslatorManager(Parameters.translator_id); // use all available
																							// translators

	private DiversiconAdaptor div = null; // DIVERSICON
...
//			
	...
	public void precessLexicon(boolean extractLabels) {
		...
		// OWLClass cls =
		// SynchronizedOWLManager.createOWLDataFactory().getOWLClass(IRI.create("http://logmap.cs.ox.ac.uk/ontologies#TopClass"));
		...
		// Useful for the multifarm task where we need to translate each ontology
		// servarl times.
		// Thus we translate for the first occurrence and we keep (locally) the
		// translation for the next cases.
			...
			if (Parameters.avoid_redundant_labels) {
				...
				for (Set<String> entry : invertedFileIndividuals.keySet()) {
					if (invertedFileIndividuals.get(entry).size() > Parameters.max_redundancy_labels) {
						...
						// System.out.println("Redundancey: " + entry + " " +
						// invertedFileIndividuals.get(entry).size());
						// }
						...
						for (int ident : invertedFileIndividuals.get(entry)) {
							...
							// index.getAlternativeLabels4IndividualIndex(ident).size() + " " +
							// index.getAlternativeLabels4IndividualIndex(ident) + " " +
							// index.getIRIStr4IndividualIndex(ident));
							// }
		...
		if (Parameters.allow_multilingual) {
			...
			if (!lang.equals("") && !lang.equals(Parameters.target_lang)) {
				...
				/*
				 * if (Parameters.is_test_mode_multilingual){ WriteFile writer = new WriteFile(
				 * "/home/ernesto/Documents/OAEI_OM_2015/EVAL_2015/error_multilingual/total_characters_reduced.txt",
				 * true); //System.out.println("Num translated characters: " +
				 * translator.getNumberOfTranslatedCharacters());
				 * //System.out.println("Num calls to google: " +
				 * translator.getNumberOfGoogleTranslateCalls());
		...
		/*
		 * int exactIFAmb=0; for (Set<String> setstr : invertedFileExact.keySet()){
		 * //LogOutput.print(setstr); if (invertedFileExact.get(setstr).size()>1)
		 * exactIFAmb++; } LogOutput.print("Ambiguity IF exact: " + exactIFAmb);
		 */
		...
		/*
		 * for (int i=1900; i<2000; i++){ if
		 * (identifier2ClassIndex.get(i).getAlternativeLabels().size()>1){
		 * LogOutput.print(i);
		...
		LogOutput.print(
				"Time setting labels and inverted files (s): " + (float) ((double) fin - (double) init) / 1000.0);
	...
	private void processLexiconDataProperties(boolean extractLabels) {
		...
		for (OWLDataProperty dProp : onto.getDataPropertiesInSignature(true)) { // also imports
			...
			for (OWLAnnotationAssertionAxiom dPropAnnAx : dProp.getAnnotationAssertionAxioms(onto)) {
				...
				for (String label_value : annotationExtractor.getAnntotationString(dPropAnnAx, onto,
						index.getFactory())) {
					...
					for (OWLClassExpression clsexpunion : clsexp.asDisjunctSet()) {
						if (!clsexpunion.isAnonymous()) {
							if (class2identifier.containsKey(clsexpunion.asOWLClass())) {
								index.addDomainClass4DataProperty(ident,
										class2identifier.get(clsexpunion.asOWLClass()));
							}
						}
					}
				}
			...
			for (OWLDataRange type : dProp.getRanges(onto)) {
				if (type.isDatatype()) {
					// LogOutput.print(type.asOWLDatatype() + " " + type.asOWLDatatype().getIRI() +
					// " " + type.asOWLDatatype().isBuiltIn() +" " +
					...
					try {
						if (type.asOWLDatatype().isBuiltIn()) {
							range_type = type.asOWLDatatype().getBuiltInDatatype().getShortName();
								...
								// range_type =
								// Utilities.getEntityLabelFromURI(Utilities.getEntityLabelFromURI(type.asOWLDatatype().getIRI().toString()));
			...
			// LogOutput.print("\tDOMAINS: " +
			// identifier2DataPropIndex.get(ident).getDomainClassIndexes());
			// LogOutput.print("\tRANGES: " +
			// identifier2DataPropIndex.get(ident).getRangeTypes());
	...
	private void processLexiconObjectProperties(boolean extractLabels) {
		...
		for (OWLObjectProperty oProp : onto.getObjectPropertiesInSignature(true)) {// also imports
			...
			for (OWLAnnotationAssertionAxiom oPropAnnAx : oProp.getAnnotationAssertionAxioms(onto)) {
				...
				// TODO Perform translation if necessary
				for (String label_value : annotationExtractor.getAnntotationString(oPropAnnAx, onto,
						index.getFactory())) {
			...
			for (OWLObjectPropertyExpression propexp : oProp.getInverses(onto)) {
				if (!propexp.isAnonymous()) {
					inverse_name = Utilities.getEntityLabelFromURI(propexp.asOWLObjectProperty().getIRI().toString());

					// Reuse name of inverse property to create an alternative label for the
					// property
			...
			for (OWLClassExpression clsexp : oProp.getDomains(onto)) {
				...
				if (!clsexp.isAnonymous()) {
					...
					for (OWLClassExpression clsexpunion : clsexp.asDisjunctSet()) {
						if (!clsexpunion.isAnonymous()) {
							if (class2identifier.containsKey(clsexpunion.asOWLClass())) {
								index.addDomainClass4ObjectProperty(ident,
										class2identifier.get(clsexpunion.asOWLClass()));
							}
						}
					}
			...
			for (OWLClassExpression clsexp : oProp.getRanges(onto)) {
				if (!clsexp.isAnonymous()) {
					...
					for (OWLClassExpression clsexpunion : clsexp.asDisjunctSet()) {
						if (!clsexpunion.isAnonymous()) {
							if (class2identifier.containsKey(clsexpunion.asOWLClass())) {
								index.addRangeClass4ObjectProperty(ident,
										class2identifier.get(clsexpunion.asOWLClass()));
							}
						}
					}
			...
			// LogOutput.print(name);
			// LogOutput.print("\tDOMAINS: " +
			// identifier2ObjPropIndex.get(ident).getDomainClassIndexes());
			// LogOutput.print("\tRANGES: " +
			// identifier2ObjPropIndex.get(ident).getRangeClassIndexes());
	...
	private void processNamedIndividuals(boolean extractLabels) {
		...
		for (OWLNamedIndividual indiv : onto.getIndividualsInSignature(true)) {// also imports
			...
			// In OAEI 2013 the ones that are referenced from property
			// "http://www.instancematching.org/label"
			...
			for (String alabel : altLabels) {
				...
				if (!isLabelAnIdentifier(alabel)) {
					...
					index.addAlternativeIndividualLabel(ident, alabel.toLowerCase()); // lower case? I guess it is the
																						// same...
					...
					if (cleanWords.size() > 0) {
						...
						if (extractLabels) {
							...
							// LogOutput.print(invertedFileExactIndividuals.get(cleanWords) + " "+cleanWords
							// + " "+ ident);
			...
			// -------------------------------
			/*
			 * String new_alabel=""; for (String altLabel :
			 * index.getAlternativeLabels4IndividualIndex(ident)){ new_alabel += altLabel +
			 * " "; } new_alabel = new_alabel.trim(); if (new_alabel.length()>3)
			...
			// *****
			// Deprecated (August 28, 2017): check
			// LogMapCore.createAndAssessInstanceMappings and
			// MappingManager.addInstanceMapping
			// *****

			// Extract class types.
			// We only extract direct types. Inferred types will be extracted later when
			// setting up the taxonomic data.
			 ...
			 * 
			 * if (!clsexp.isAnonymous()){ if
			 * (class2identifier.containsKey(clsexp.asOWLClass())){
			 * index.addType4Individual(ident, class2identifier.get(clsexp.asOWLClass()));
			 * 
			 * 
			 * 
			 * if (Parameters.allowed_instance_types.contains(clsexp.asOWLClass().getIRI().
			 * toString())) showIndividualInOutput=true; } } else if
			 * (clsexp.getClassExpressionType()==ClassExpressionType.OBJECT_UNION_OF){ for
			 * (OWLClassExpression clsexpunion : clsexp.asDisjunctSet()){ if
			 * (!clsexpunion.isAnonymous()){ if
			 * (class2identifier.containsKey(clsexpunion.asOWLClass())){
			 * index.addType4Individual(ident,
			 * class2identifier.get(clsexpunion.asOWLClass())); if
			 * (Parameters.allowed_instance_types.contains(clsexp.asOWLClass().getIRI().
			 * toString())) showIndividualInOutput=true; } } } } //other not considered yet
			 * 
			 * }
			 * 
			 * //IN SOME INSTANCE MATCHING TASKS ONLY IT IS REQUIRED TO DISCOVER INDIVIDUAL
			 * OF CERTAIN CLASSES index.setShowInOutput4Individual(ident,
			 * showIndividualInOutput);
			 */
			// Deprecated
			...
			/*
			 * //LogOutput.print("\t" + indiv.getIRI().toString()); //LogOutput.print("\t" +
			 * index.getName4IndividualIndex(ident)); //LogOutput.print("\t" +
			 * index.getLabel4IndividualIndex(ident)); LogOutput.print("\t" +
			 * index.getIRIStr4IndividualIndex(ident)); LogOutput.print("\t" + altLabels);
			 * 
			 * LogOutput.print("\tTYPES: " +
			 * index.getIndividualClassTypes4Identifier(ident));
			 * LogOutput.print("\tIF size: " + invertedFileExactIndividuals.size()); if
			 * (invertedFileExactIndividuals.size()<5){ LogOutput.print("\tIF size: " +
			 * invertedFileExactIndividuals); }
			 */
		...
		// Another for to extract lexicon from other labels (relationships with other
		// indiv or data)
		...
		if (extractLabels) {
			...
			for (OWLNamedIndividual indiv : onto.getIndividualsInSignature(true)) {// also imports
				...
				// We extract the set of referenced individuals: Cities for lives_in, Paper for
				// author_of, etc
				Map<OWLObjectPropertyExpression, Set<OWLIndividual>> objProp2values = indiv
				...
				for (OWLObjectPropertyExpression objprop : objProp2values.keySet()) {
					...
					for (OWLIndividual indiv_deep2 : objProp2values.get(objprop)) {
						...
						index.addReferencedIndividual4Individual(inidividual2identifier.get(indiv),
								inidividual2identifier.get(indiv_deep2));
					}
				} // for obj prop
				...
				index.addCharactersitic4Individual(inidividual2identifier.get(indiv),
						oaei2015extractor.getPublicationCount(onto, indiv));
				// Characteristic 2: Number of citations
				index.addCharactersitic4Individual(inidividual2identifier.get(indiv),
						oaei2015extractor.getNumberOfCitations(onto, indiv));
				// Characteristic 3: active from
				index.addCharactersitic4Individual(inidividual2identifier.get(indiv),
						oaei2015extractor.getActiveFromYear(onto, indiv));

				// if
				// (index.getIdentifier2IndividualIndexMap().get(inidividual2identifier.get(indiv)).showInOutput())
				// LogOutput.printAlways(indiv.getIRI() + " referred: " +
				// index.getReferencedIndividuals4Individual(inidividual2identifier.get(indiv)).size());
	...
	private Set<String> extractNameFromDataAssertion(OWLNamedIndividual indiv) {
		...
		for (String uri_indiv_ann : Parameters.accepted_data_assertion_URIs_for_individuals) {
			...
			for (OWLLiteral assertion_value : indiv
					.getDataPropertyValues(index.getFactory().getOWLDataProperty(IRI.create(uri_indiv_ann)), onto)) {
	...
	private Set<String> extractAnnotations4Infividual(OWLNamedIndividual indiv) {
		...
		for (OWLAnnotationAssertionAxiom indivAnnAx : indiv.getAnnotationAssertionAxioms(onto)) {
			...
			// String uri_ann =
			// indivAnnAx.getAnnotation().getProperty().getIRI().toString();
	...
	/**
	 * This processes the property label and alternative labels, and create inverted
	 * files Used for properties and individuals. Classes have a more sophisticated
	 * processing
	 * 
	 * @param label
	 * @param ident
	...
	private List<String> processLabel(String label, boolean filterStopwords) {
		...
		for (int i = 0; i < words.length; i++) {
			...
			if (words[i].length() > 0 && (!filterStopwords || !lexicalUtilities.getStopwordsSet().contains(words[i]))) {
				// if (words[i].length()>0 && (!filterStopwords ||
	...
	/**
	 * This class processes the label of P2 (an inverse property of P1) and returns
	 * a candidate alternative label for P1. e.g: hasPart -> partOf e.g: authorOf ->
	 * hasAuthor e.g. writtenBy -> written (would be similar to writes) e.g.
	 * isGivenBy ->given (would be closer to gives)
	 * 
	 * @param label
	 * @return
	 */
	...
	/**
	 * Create alternative label for a given property e.g. hasName -> name e.g.
	 * isReviewedBy - > hasReviewer
	 * 
	 * @param label
	 * @return
	 */
	...
	/**
	 * This methid extracts and processes the class label and alternative labels,
	 * and create inverted files
	 * 
	 * @param cls
	 */
	private void createEntryInLexicalInvertedFiles4ClassLabels(OWLClass cls, int ident) {
		...
		for (String str : cleanWords) {
			...
			// LogOutput.print(str + "-> stemming: '" +
			// lexicalUtilities.getStemming4Word(str) +"'");
			// }
		...
		for (String altlabel_value : extractAlternateLabels4OWLCls(cls, ident)) {
			...
			// TODO Lower case altlabel_value?? And not to lower case in annotation
			// extraction??
			...
			if (altlabel_value.length() > 2) {
				...
				for (int i = 0; i < words.length; i++) {
					...
					if (!lexicalUtilities.getStopwordsSet().contains(words[i]) && words[i].length() > 0) { // words[i].length()>2
																											// && Not
																											// for exact
																											// if: it
																											// may
						...
						/*
						 * if (frequency4words.containsKey(words[i])) frequency4words.put(words[i], 1);
						 * else frequency4words.put(words[i], frequency4words.get(words[i]) + 1);
						 */
				...
				for (String str : cleanWords) {
					...
					// if (lexicalUtilities.getStemming4Word(str).equals("")){
					// LogOutput.print(str + "-> stemming: '" +
					// lexicalUtilities.getStemming4Word(str) + "'" + stemmedWord.isEmpty());
					// }
	...
	private boolean isLabelAnIdentifier(String label_value) {
		...
		// || label_value.matches("[0-9]+\\.[0-9]+(\\.[0-9]+)+");//library ontologies
				|| label_value.matches("[0-9]+(\\.[0-9]+)+") || label_value.matches(".+[0-9]+.+[0-9]+.+[0-9]+.+") // instance
																													// matching
																													// ontologies
	...
	private Set<String> extractCleanLabel4OWLCls(OWLClass cls, int ident) {
		 ...
		 * 
		 * //Ambiguity in labels of ontology (use concept name) if
		 * (extact_occurrence_entries.containsKey(entry4ifexact)){
		 * 
		 * ambiguity_ident=extact_occurrence_entries.get(entry4ifexact); //We remove
		 * previous one extact_occurrence_entries.remove(entry4ifexact);
		 * 
		 * //We create two news
		 * extact_occurrence_entries.put(getProcessedName4ConceptIndex(ambiguity_ident),
		 * ambiguity_ident);
		 * extact_occurrence_entries.put(getProcessedName4ConceptIndex(ident), ident);
		 * 
		 * 
		 * 
		 * } else{ extact_occurrence_entries.put(entry4ifexact, ident); } }
		 */
	...
	/**
	 * Return alternate labels They could be represented using different annotation
	 * properties
	 * 
	 * @param cls
	 * @return
...
//			diversiconReturnedSynonyms = div.getRelatedWords(null, null, words[i], IDivAPI.WORD_SIMILARITY);
	...
	/**
	 * Combines the words in given list with 'x' missing words and stores the
	 * results in IF
	 * 
	 * @param cleanWords
	 *            Clean label of concept
	 * @param ident
	 *            Identifier of concepts
	 * @param missing_words
	 *            Number of words to be discarded
	 */
	private void createWeakLabels4Identifier(List<String> cleanWords, int ident, int missing_words) {
		...
		// Set<Set<Integer>> combination_set =
		// getIdentifierCombination(cleanWords.size(), missing_words);
		Set<Set<Integer>> combination_set = precomputeIndexCombination.getIdentifierCombination(cleanWords.size(),
				missing_words);
	...
	public void setTaxonomicData() {
		...
		/*
		 * if (isOntoPreclassified){ init=Calendar.getInstance().getTimeInMillis();
		 * unloadOntology(); loadOWLOntology(physical_iri_class); fin =
		 * Calendar.getInstance().getTimeInMillis(); LogOutput.
		 * print("Time unloading Ontology and loading pre-classified ontology (s): " +
		...
		// fin = Calendar.getInstance().getTimeInMillis();
		// LogOutput.print("Time classifying ontology (s): " +
		// (float)((double)fin-(double)init)/1000.0);
		...
		// fin = Calendar.getInstance().getTimeInMillis();
		// LogOutput.print("Time Extracting Taxonomy (s): " +
		// (float)((double)fin-(double)init)/1000.0);
	...
	/*
	 * MORe or Structural reasoner private void setUpReasoner_MORe() {
	 * 
	 * MOReAccess moreAccess=null;
	 * 
	 * //Try first with Hermit if classifies in more than x seconds or error then
	 * structural
	 * 
	 * try {
	 * 
	 ...
	 * SynchronizedOWLManager.createOWLOntologyManager(), onto, false); //with
	 * factory i got problems
	 * 
	 * 
	 * //Timeout is not properly handled with MORe
	 * //moreAccess.classifyOntology_withTimeout_throws_Exception(17); //We then do
	 * the timeout internally in MORe only over HermiT (or the OWL 2 reasoner)
	 * //MORe access invokes a version or MORe with timeout
	 * //moreAccess.classifyOntology(false); //no props
	 * moreAccess.classifyOntology_withTimeout_throws_Exception(Parameters.timeout);
	 * //in case of error or timeout
	 * 
	 * 
	 * if (moreAccess.isOntologyClassified()){ reasoner = moreAccess.getReasoner();
	 * } else { LogOutput.
	 * print("Onto not classified with MORe. Using 'structural' reasoner instead.");
	 * reasoner = getIncompleteReasoner(); }
	 * 
	 * if (reasoner==null){
	 ...
	 * 
	 * try { LogOutput.
	 * print("Error/timeout setting up MORe reasoner. Using 'structural' reasoner instead."
	 * );//\n\n" + e.getMessage() + "\n");
	 * 
	 * reasoner = getIncompleteReasoner(); } catch(Exception e2){
	 * System.err.println("Error setting up Structural reasoner: " +
	 * e2.getMessage()); e2.printStackTrace(); }
	 * 
	 * }
	 * 
	 * //if (reasoner.getReasonerName()!=null){ //LogOutput.print("Reasoner name: "
	 * + reasoner.isConsistent() + ".\n"); //LogOutput.print("Reasoner name: " +
	 * reasoner + ".\n"); //LogOutput.print("Reasoner name: " +
	...
	private void setUpReasoner_HermiT() {
		...
		try {
			...
			if (Parameters.reason_datatypes || onto.getDatatypesInSignature(true).size() == 0) {
				...
				hermitAccess = new HermiTAccess(SynchronizedOWLManager.createOWLOntologyManager(), onto, false); // with
																													// factory
																													// i
																													// got
																													// problems

				hermitAccess.classifyOntology_withTimeout_throws_Exception(Parameters.timeout); // in case of error or
																								// timeout
	...
	private void setUpReasoner_ELK() {
		...
		// that provides is less complete than the structural reasoner
		// when the ontology is not EL!!!
		if (!profileChecker.isInOWL2ELProfile(onto)) {
			LogOutput.printAlways(
					onto.getOntologyID().getOntologyIRI() + " NOT in OWL 2 EL profile. Using structural reasoner.");
			setUpStructuralReasoner();
			return;
		}
		...
		// Try first with ELK if classifies in more than x seconds or error then
		// structural

		try {

			elkAccess = new ELKAccess(SynchronizedOWLManager.createOWLOntologyManager(), onto, false); // with factory i
																										// got problems
	...
	private OWLReasoner getIncompleteReasoner() throws Exception {
		 ...
		 * SynchronizedOWLManager.createOWLOntologyManager(), onto, false); return
		 * elk.getReasoner();
		 */
	}
	...
	/**
	 * Create taxonomy (for interval labeling), and disjointness and equivalence
	...
	private void extractStringTaxonomiesAndDisjointness() {
		...
		// TOP CONCEPTS
		NodeSet<OWLClass> topClasses = reasoner.getSubClasses(reasoner.getTopClassNode().getRepresentativeElement(),
				true);
		// Real root nodes
		...
		// All Subclasses
		NodeSet<OWLClass> nodeSet = reasoner.getSubClasses(reasoner.getTopClassNode().getRepresentativeElement(),
				false);

		// LogOutput.print(reasoner.getTopClassNode().getRepresentativeElement() + " " +
		// nodeSet.getNodes().size());
		...
		// MEANINGFUL ROOTS: to avoid cases like SNOMED where there is an unique root
		// apart from THING
		...
		// extractHierarchyLevel(reasoner.getSubClasses(reasoner.getTopClassNode().getRepresentativeElement(),
		// true), 1);
		...
		for (Node<OWLClass> node : nodeSet) {
			...
			if (!reasoner.getSubClasses(clsRepresentative, true).isEmpty()) {
				...
				for (Node<OWLClass> nodeSub : reasoner.getSubClasses(clsRepresentative, true).getNodes()) {
					...
					// identifier2directkids.get(identRepresentative).add(node2identifier.get(nodeSub));//will
					// give us direct kid identifiers
			...
			if (!reasoner.getSuperClasses(clsRepresentative, true).isEmpty()) {
				...
				for (Node<OWLClass> nodeSup : reasoner.getSuperClasses(clsRepresentative, true).getNodes()) {
					...
					// identifier2directparents.get(identRepresentative).add(node2identifier.get(nodeSup));
					// //direct parents
					// System.out.println(identRepresentative + " " + nodeSup + " " +
					// node2identifier.get(nodeSup));
			...
			for (OWLClass nodeClass : node.getEntities()) { // for all node classes
				...
				// We check for each class. differnt disjoint axioms may affect different
				// classes of same node
				...
				for (int i = 0; i < nodeClasses.length; i++) {
					...
					for (int j = 0; j < nodeClasses.length; j++) {
						...
						if (index.getClassIndex(ident1).hasDirectDisjointClasses()) {
							...
							index.getClassIndex(ident2)
									.addAllDisjointClasses(index.getClassIndex(ident1).getDisjointClasses());
						}
	...
	/**
	 * Extract axioms of the form A^...^B- > C from subclassof or equivalence axioms
	 */
	...
	private void extractGeneralHornAxioms() {
		...
		/*
		 * int i=0; String horn=""; for (Set<Integer> body :
		 * generalHornAxioms.keySet()){ i++;
		 * 
		 * 
		 * for (int b : body){ horn+=index.getClassIndex(b).getLabel() + " ^ "; }
		 * 
	...
	private void extractInferredTypes4Individuals() {
		...
		for (int identIndiv : index.getIndividuaIdentifierSet()) {
			...
			for (Node<OWLClass> node_cls : reasoner.getTypes(index.getOWLNamedIndividual4IndividualIndex(identIndiv),
					true)) {
	...
	private void addOWLClassExpresion2GeneralHornAxiom(int idecls, OWLClassExpression exp) {
		...
		if (exp instanceof OWLObjectIntersectionOf) {
			...
			for (OWLClassExpression exp_intersect : ((OWLObjectIntersectionOf) exp).getOperands()) {
				...
				// if (exp_intersect.isClassExpressionLiteral()){ //A and notA are considered
	...
	/*
	 * Set with representative for equivalence nodes Will be required after interval
	 * labelling indexing
	 * 
	 * @return
	 * 
	 * public Set<Integer> getRepresentativeNodes(){ return representativeNodes; }
	 */
	 ...
	 * @return
	 *
	 * public Set<Integer> getRootNodes(){ return RootIdentifiers; }
	 */
	 ...
	 * 
	 * for (int ideroot : MeaningfulRootIdentifiers){ if (isSubClassOf(ident,
	 * ideroot)){ set.add(ideroot); } }
	 * 
	 * return set;
	 * 
	 * }
	...
	private void extractHierarchyLevel(NodeSet<OWLClass> classes, int level) {
		...
		// TOP CONCEPTS
		// NodeSet<OWLClass> topClasses =
		// reasoner.getSubClasses(reasoner.getTopClassNode().getRepresentativeElement(),
		// true);
	...
	public void buildFilteredInvertedIndex(Set<String> stopWords) {
		...
		/*
		 * Set<String> specialKey = new HashSet<String>(); specialKey.add("vas");
		 * specialKey.add("va"); specialKey.add("def");
		 */
		 ...
		 * 
		 * Entry<Set<String>, Set<Integer>> key1 = null, key2; for (Entry<Set<String>,
		 * Set<Integer>> entry : temp) { Lib.logInfo("- " + entry.getKey().toString());
		 * if (entry.getKey().equals(specialKey)) { if (key1 == null)
		 * LogOutput.print(key1 = entry); else { LogOutput.print(key2 = entry);
		 * LogOutput.print(key1.equals(key2) + " " + key1.hashCode() + " " +
		 * key2.hashCode()); LogOutput.print(key1 + " "+ key2); } }
		 * 
		 * }
		 * 
		 * Lib.closeLog();
			...
			for (OWLAnnotationAssertionAxiom entityAnnAx : indiv.getAnnotationAssertionAxioms(onto)) {
				...
				if (Parameters.accepted_property_URIs_for_instance_categories.contains(uri_ann)) {
					...
					if (!(category_value = asDirectValue(entityAnnAx)).equals("")) {
						...
						// Cross terminologies
						// if
						// (category_value.equals("http://data.doremus.org/vocabulary/diabolo/genre/lied"))
						// index.addCategory4Individual(ident,
						// "http://data.doremus.org/vocabulary/iaml/genre/li");
						// Narrower-broader relationships
						// if
						// (category_value.equals("http://data.doremus.org/vocabulary/iaml/genre/opc"))
						// index.addCategory4Individual(ident,
				...
				try {
					for (OWLIndividual assertion_value_indiv : indiv.getObjectPropertyValues(
						...
						if (assertion_value_indiv.isNamed()) {
							...
							if (category_value != null && !category_value.equals("null")
									&& !category_value.equals("")) {
								index.addCategory4Individual(ident, category_value);
								continue;
							}
	...
	/**
	 * 
	 * Manages the extraction of the lexicon associated to an Individual that is
	 * presented as data asserion axioms or object asssertion axioms. Currently is
	 * based on the IM track of the OAEI 2012
	 * 
	 * @author Ernesto
	 *
	 */
	private class ExtractAcceptedLabelsFromRoleAssertions {
		...
		protected boolean isDummyIndividual(OWLNamedIndividual indiv) {
			...
			// -------------------------------------------------------------------------
			// If referenced it is a dummy individual which should not be considered in the
			// matching
		...
		protected Set<String> extractLexiconFromRoleAssertions(OWLNamedIndividual indiv) {
			...
			for (String uri_indiv_ann : Parameters.accepted_data_assertion_URIs_for_individuals) {
				...
				for (OWLLiteral assertion_value : indiv.getDataPropertyValues(
					...
					label_value = processLabel(assertion_value.getLiteral().toLowerCase());
			...
			for (String uri_indiv_ann_deep1 : Parameters.accepted_object_assertion_URIs_for_individuals) {
				...
				for (OWLIndividual assertion_value_indiv : indiv.getObjectPropertyValues(
					...
					if (assertion_value_indiv.isNamed()) {
						...
						//// DBPedia references
						// if
						//// (assertion_value_indiv.asOWLNamedIndividual().getIRI().toString().contains("dbpedia.org")){
						// label_value =
						//// processLabel(Utilities.getEntityLabelFromURI(assertion_value_indiv.asOWLNamedIndividual().getIRI().toString()));
						// }
						// else{
						...
						for (String uri_indiv_ann_deep2 : Parameters.accepted_data_assertion_URIs_for_individuals_deep2) {
							...
							for (OWLLiteral assertion_value_deep2 : assertion_value_indiv.asOWLNamedIndividual()
									.getDataPropertyValues(
											index.getFactory().getOWLDataProperty(IRI.create(uri_indiv_ann_deep2)),
											onto)) {

								label_value = processLabel(assertion_value_deep2.getLiteral().toLowerCase());
						...
						// ---------------------
						for (OWLAnnotationAssertionAxiom indivAnnAx_level2 : assertion_value_indiv
								.asOWLNamedIndividual().getAnnotationAssertionAxioms(onto)) {

							String uri_ann = indivAnnAx_level2.getAnnotation().getProperty().getIRI().toString();

							if (Parameters.rdf_comment_uri.equals(uri_ann)
									|| Parameters.rdf_label_uri.equals(uri_ann)) {

								try {
									label_value = processLabel(
											((OWLLiteral) indivAnnAx_level2.getAnnotation().getValue()).getLiteral()
													.toLowerCase());
								} catch (Exception e) {
									// In case of error.
									label_value = "";
		...
		/**
		 * This method extracts lexicon from other properties. Like "lives in X". In the
		 * end we will only keep those relationships that uniquely identifies an
		 * instance
		 * 
		 * @param indiv
		 * @return
		...
		protected Set<String> extractExtendedLexiconFromRoleAssertions(OWLNamedIndividual indiv) {
			...
			for (OWLDataPropertyExpression dataprop : dataProp2values.keySet()) {
				...
				if (Parameters.accepted_data_assertion_URIs_for_individuals
						.contains(dataprop.asOWLDataProperty().getIRI().toString())
				...
				for (OWLLiteral literal : dataProp2values.get(dataprop)) {
					...
					// We also normalize dates in case the give string is within one of the
					// acceptred date formats
					lexiconValues4individual
							.add(label_name + "_" + NormalizeDate.normalize(literal.getLiteral().toString()));
			...
			for (OWLObjectPropertyExpression objprop : objProp2values.keySet()) {
				...
				for (OWLIndividual indiv_deep2 : objProp2values.get(objprop)) {
					...
					// If has alternative labels, otherwise we cannot do much (since label will
					// probably be a "randomly" generated identifier)
					...
					if (index.hasIndividualAlternativeLabels(ident)) {
						...
						// We estract a data props lecicon like "date_12_12_2012", "age_18", etc...
						// in OAEI 2013 I this is not used much... since deep2 obj properties substitute
						// another obj propertyes
						for (OWLDataPropertyExpression dataprop : dataProp2values.keySet()) {

							if (dataprop.isAnonymous())
								continue;

							label_name = Utilities
									.getEntityLabelFromURI(dataprop.asOWLDataProperty().getIRI().toString());

							for (OWLLiteral literal : dataProp2values.get(dataprop)) {

								// if (isGoodLabel(literal.getLiteral().toString()))
								lexiconValues4individual.add(
										label_name + "_" + NormalizeDate.normalize(literal.getLiteral().toString()));

							}

						}

						Map<OWLObjectPropertyExpression, Set<OWLIndividual>> objProp2values_deep2 = indiv_deep2
								.getObjectPropertyValues(onto);
						...
						for (OWLObjectPropertyExpression objectprop2 : objProp2values_deep2.keySet()) {
							...
							label_name = Utilities
									.getEntityLabelFromURI(objectprop2.asOWLObjectProperty().getIRI().toString());
							...
							for (OWLIndividual indiv_deep3 : objProp2values_deep2.get(objectprop2)) {
								...
								if (index.hasIndividualAlternativeLabels(ident2)) {
									lexiconValues4individual
											.add(label_name + "_" + index.getLabel4IndividualIndex(ident2));
								}
							}
		...
		public String processLabel(String value) {
			...
			// Reg expression to split text: //Split up to "&", ".", "(", "," ";", is, was,
			// are, were, est/fut (french)
			// In this way we split long comments and we only get the important part as
			// synonym
			// String reg_ex_split = "[&\\.,;(]";
			...
			if (processedLabel.length() <= manegeable_lenght && !processedLabel.contains("<p>")
					&& !processedLabel.contains("</p>")) {

				// We still want to split and trim
				// we keep/split string up to the given character (twice in case mroe than one
				// character)
				...
				/*
				 * if (!isGoodLabel(processedLabel)){ //LogOutput.printAlways("Filtered: "+
				 * processedLabel); return ""; //bad label } else{ //LogOutput.print("GOOD: "+
				 * processedLabel); }
				 */
		...
		private boolean isGoodLabel(String label) {
			...
			// we do not consider "y" as consonant for filtering purposes. After all "y" may
			// have vowel phonetics.
			...
			String more5_consonants_regex = consonant_regex + consonant_regex + consonant_regex + consonant_regex
					+ consonant_regex + consonant_regex + "+";
			...
			for (String word : words) {
				...
				if (word.length() > 1
						&& NormalizeNumbers.getRomanNumbers10().contains(word.substring(0, word.length() - 1))) {
					// LogOutput.print(word + " " + word.substring(0, word.length()-1) + " " +
				...
				// We now accept single letetrs in labels. Specially for names: i,e, "v
				// carapella"
				...
				// Strange words
				// In english it is very rare to have more than 4 consonant together (e.g.
				// angsts). In compounds words 5 it is possible (e.g. handspring)
				// Starting the word cluster is three consonants is the maximum number, as in
				// "split".
				// More than 3 vowels together is also strange
				if (!word.startsWith("mc") && word.matches(more3_consonants_regex + ".*")) { // Starting with more thn 3
																								// cons. //Mc surname
																								// like McGregor include
					return false;
				}
	...
	public static void main(String[] args) {
		...
		p.roleAssertionLabelsExtractor.processLabel(
